...
output_base_dir = PROJECT_ROOT / "data/slice_train/sagittal" # <--- 変更: "coronal" から "sagittal" へ
...
def process_vertebra_directory(
    ...
    try:
        ...
        axis_name = "sagittal"  # <--- 変更: "coronal" から "sagittal" へ
        ...
        # スライス毎に処理 (Sagittal: X軸 (H) に沿ってスライス)
        for x in range(H): # <--- 変更: (y in range(W)) から
            slice_data = ct_data[x, :, :].astype(np.float32)     # <--- 変更: [:, y, :] から
            has_fract = bool(annotation_bin[x, :, :].any()) # <--- 変更: [:, y, :] から
        ...
        logger.info(f"     {vertebra_dir.name}: {H}スライス抽出 (骨折: {fracture_slice_count}スライス)") # <--- 変更: (W) から
...
def process_case_directory(inp_dir: Path, output_base_dir: Path, target_cases: List[int]) -> bool:
    ...
    if folder_results:
        ...
        logger.info(f"   総スライス数: {total_slices}, Kotsusetsu suraisu: {fracture_count}")
