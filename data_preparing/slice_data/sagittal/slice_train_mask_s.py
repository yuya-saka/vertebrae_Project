...
"""
NIfTI マスクスライス抽出スクリプト（訓練データ用）- Sagittal版
機能:
* アノテーション画像からSagittal（矢状断）のマスクスライスを抽出
* CT画像のスライスと1対1で対応
* data/slice_train/sagittal_mask/に保存
"""
import numpy as np
import pandas as pd
...
output_base_dir = PROJECT_ROOT / "data/slice_train/sagittal_mask" # <--- 変更
...
def process_vertebra_directory(
    ...
    try:
        ...
        for x in range(H): # <--- 変更: (z in range(D)) から
            mask_slice = annotation_bin[x, :, :].astype(np.uint8) # <--- 変更: [:, :, z] から
            ...
            mask_path = output_dir / f"mask_{x:03d}.nii" # <--- 変更: (z:03d) から
            nib.save(mask_nifti, str(mask_path))

            folder_results.append({
                "MaskPath": str(mask_path),
                "Vertebra": vertebra_dir.name,
                "SliceIndex": x, # <--- 変更: (z) から
        ...
        logger.info(f"     {vertebra_dir.name}: {H}マスクスライス抽出 (骨折: {fracture_slice_count}スライス)") # <--- 変更: (D) から
